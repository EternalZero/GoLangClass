...
const gcsBucket = "alien-emblem-128723.appspot.com"

func init() {
	http.HandleFunc("/", handler)
	http.HandleFunc("/raw", retriever)
}
...
func handler(res http.ResponseWriter, req *http.Request) {
	...
	if req.Method == "POST" {
		...
		for k, _ := range fnames {
			html += `<h3><a href="/raw?object=` + k + `">` + k + `</a></h3>`
		}
	}
